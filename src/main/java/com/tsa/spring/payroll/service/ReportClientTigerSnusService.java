...
import com.tsa.spring.payroll.Utils.ExcelFormulaHelperTigerSnus;
...
import com.tsa.spring.payroll.repository.ReportClientTigerSnusRepo;
...
@Service
@Transactional
public class ReportClientTigerSnusService {
    ...
    public void exportClientTigerSnus(HttpServletResponse response, SearchData searchData)throws Exception{

        Specification<ReportClientTigerSnus> spec = reportClientSpecification.searchReportClient(searchData);
        List<ReportClientTigerSnus> dataClientTigerSnus = reportClientTigerSnusRepo.findAll(spec);

        response.setContentType("application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
        response.setHeader("Content-Disposition", "attachment; filename=ReportClientTigerSnus.xlsx");

        ClassPathResource temPathResource = new ClassPathResource("templates/excel/TemplateTigerSnus.xlsx");
        ...
        int lastRow = startRowIndex + dataClientTigerSnus.size();
        Row sumRow = sheet.createRow(lastRow);
        Set<Integer>formulaSumColumns = new HashSet<>();
        for(int i = 15; i <= 36; i++)formulaSumColumns.add(i);
        

        for(int i =0; i < dataClientTigerSnus.size(); i++){

            ReportClientTigerSnus data = dataClientTigerSnus.get(i);
            ...
            for (int colIndex = 1; colIndex <= 38; colIndex++) {
                ...
                if (formulaColumns.contains(colIndex)) {
                    String formula = ExcelFormulaHelperTigerSnus.generateFormula(colIndex, startRowIndex + i + 1);
            ...
            for(int colIndex = 1; colIndex <= 38; colIndex++){
                ...
                if(formulaSumColumns.contains(colIndex)){
                    String formula = ExcelFormulaHelperTigerSnus.generateTotalFormula(colIndex, startRowIndex + 1, lastRow);
